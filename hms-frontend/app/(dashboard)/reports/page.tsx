...
import {
  DollarSign, TrendingUp, CalendarCheck, Clock, Star,
  Download, Printer, FileText, ArrowUpRight, ArrowDownRight, X
} from "lucide-react";
import {
...
// ── Summary Card ──────────────────────────────────────────────────────────
function SummaryCard({ title, value, delta, icon: Icon, color, bg }: {
  ...
  return (
    <div className="bg-theme-card shadow-soft border border-theme-border rounded-2xl p-5 flex flex-col gap-3 flex-1 min-w-[160px]">
      <div className="flex items-center justify-between">
        <span className="text-xs text-theme-muted font-medium uppercase tracking-wider">{title}</span>
...
// ── Period Dropdown ────────────────────────────────────────────────────────
function PeriodSelect({ value, onChange, options = ["Daily", "Weekly", "Monthly"] }: {
...
// ── Full Report Modal ──────────────────────────────────────────────────────
function FullReportModal({ title, headers, rows, totalRow, onClose }: {
  title: string;
  headers: string[];
          ...
          <table className="w-full text-left text-xs min-w-max">
            <thead>
              <tr className="border-b border-theme-border bg-theme-main sticky top-0">
                {headers.map((h, i) => (
                  <th key={i} className="px-4 py-3 text-theme-muted font-semibold uppercase tracking-wider whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="text-theme-muted">
              {rows.map((row, ri) => (
                <tr key={ri} className="border-b border-theme-border/60 hover:bg-theme-hover/30 transition-colors">
                  {row.map((cell, ci) => (
              ...
              {totalRow && (
                <tr className="bg-theme-main border-t-2 border-theme-border font-bold text-theme-text sticky bottom-0">
                  {totalRow.map((cell, ci) => (
                    <td key={ci} className="px-4 py-3 whitespace-nowrap">{cell}</td>
                  ))}
                </tr>
              )}
            </tbody>
...
// ── Inline Data Table ──────────────────────────────────────────────────────
function DataTable({ title, headers, rows, totalRow, onViewFull }: {
  title: string;
  headers: string[];
  rows: (string | number | React.ReactNode)[][];
  totalRow?: (string | number | React.ReactNode)[];
  ...
  return (
    <div className="bg-theme-card shadow-soft border border-theme-border rounded-2xl overflow-hidden flex flex-col flex-1 min-w-[280px]">
      <div className="p-4 border-b border-theme-border flex items-center justify-between shrink-0">
        <h3 className="text-sm font-bold text-theme-text">{title}</h3>
        <button onClick={onViewFull} className="text-xs text-blue-400 hover:text-blue-300 transition-colors underline underline-offset-2">
          View Full Report
        </button>
      </div>
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-theme-border bg-theme-main">
              {headers.map((h, i) => (
                <th key={i} className="px-3 py-2.5 text-theme-muted font-semibold uppercase tracking-wider whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="text-theme-muted">
...
export default function ReportsPage() {
  ...
  const fetchAll = useCallback(() => {
    ...
    fetchJSON(`${API}/bookings-overview${qs}`).then(setBookings).catch(console.error);
    fetchJSON(`${API}/restaurant-revenue${qs}`).then(setRestaurant).catch(console.error);
    fetchJSON(`${API}/staff-performance${qs}`).then(setStaffPerf).catch(console.error);
    fetchJSON(`${API}/revenue-by-department${qs}`).then(setRevByDept).catch(console.error);
    fetchJSON(`${API}/guest-satisfaction`).then(setSatisfaction).catch(console.error);
    fetchJSON(`${API}/revenue-report-table${qs}`).then(setRevTable).catch(console.error);
  }, [qs]);

  useEffect(() => { fetchAll(); }, [fetchAll]);

  // ── Department filter: filter revenue chart data ──────────────────────
  const filteredRevenue = useMemo(() => revenue.map(d => {
    if (department === "Rooms") return { ...d, revenue: d.roomRevenue };
    if (department === "Restaurant") return { ...d, revenue: d.restaurantRevenue };
    return d;
  }), [revenue, department]);

  const filteredRevByDept = useMemo(() => revByDept
    ? {
        ...revByDept,
        data: department === "All Departments"
          ...
          : revByDept.data?.filter((d: any) => {
              ...
              if (department === "Restaurant") return d.name === "Restaurant";
              return true;
            }),
        total: department === "All Departments"
          ? revByDept.total
          : revByDept.data?.find((d: any) => d.name === (department === "Rooms" ? "Rooms" : "Restaurant"))?.value || 0,
      }
    : null, [revByDept, department]);
  ...
  const { occRows, occTotalRow } = useMemo(() => {
    const r = (occupancy?.table || []).map((row: any) => [
      row.date, row.occupied, row.available, row.reserved,
      <span key={row.date} className={`font-bold ${parseFloat(row.occupancyPct) >= 70 ? "text-emerald-400" : "text-amber-400"}`}>{row.occupancyPct}%</span>,
    ]);
    const t = occupancy?.table?.length
      ? [
          ...
          <span key="occ" className="text-emerald-400">{occupancy?.occupancyRate}%</span>,
        ]
      : undefined;
  ...
  const { staffRows, staffAvg } = useMemo(() => {
    const r = staffPerf.map((s: any, i: number) => [
      <span key={i} className="text-blue-400 font-medium">{s.name}</span>,
      s.department,
      s.tasksCompleted,
      `${s.efficiency}%`,
      s.rating,
    ...
    const t = staffPerf.length
      ? [
          "Average", "—",
          Math.round(staffPerf.reduce((a: any, s: any) => a + s.tasksCompleted, 0) / staffPerf.length),
          `${(staffPerf.reduce((a: any, s: any) => a + s.efficiency, 0) / staffPerf.length).toFixed(1)}%`,
          (staffPerf.reduce((a: any, s: any) => a + s.rating, 0) / staffPerf.length).toFixed(1),
        ]
      : undefined;
    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-start flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold text-theme-text">Reports & Analytics</h1>
            <p className="text-sm text-theme-muted mt-0.5">Comprehensive insights and performance metrics</p>
          </div>
        ...
        {/* ── Summary Cards ─────────────────────────────────────────── */}
          <div className="flex gap-4 flex-wrap">
            ...
            <SummaryCard title="Total Bookings" value={summary?.totalBookings ?? 0}
              delta={summary?.bookingsDelta ?? 0} icon={CalendarCheck} color="text-purple-400" bg="bg-purple-500/10" />
            <SummaryCard title="Average Length of Stay" value={summary ? `${summary.avgLOS} Nights` : "0 Nights"}
              delta={summary?.losDelta ?? 0} icon={Clock} color="text-amber-400" bg="bg-amber-500/10" />
            <SummaryCard title="Guest Satisfaction" value={summary ? `${summary.guestSatisfaction} / 5` : "4.5 / 5"}
              delta={summary?.satisfactionDelta ?? 0} icon={Star} color="text-yellow-400" bg="bg-yellow-500/10" />
          </div>

          {/* ── Row 1 Charts ──────────────────────────────────────────── */}
          <div className="flex gap-4 flex-wrap">
            {/* Revenue Trend — respects department filter */}
            <ChartCard title={department === "All Departments" ? "Revenue Trend" : `Revenue Trend (${department})`}
              action={<PeriodSelect value={revPeriod} onChange={setRevPeriod} />}>
              <ResponsiveContainer width="100%" height={210}>
                <LineChart data={filteredRevenue}>
                  ...
                  {department === "All Departments" && (
                    <>
                      <Line type="monotone" dataKey="roomRevenue" name="Rooms Revenue" stroke="#3b82f6"
                        strokeWidth={1.5} dot={false} strokeDasharray="4 4" />
                      <Line type="monotone" dataKey="restaurantRevenue" name="Restaurant Revenue" stroke="#22c55e"
                        strokeWidth={1.5} dot={false} strokeDasharray="4 4" />
                    </>
                  )}
                </LineChart>
            ...
            <ChartCard title="Occupancy Rate">
              <div className="flex items-center gap-6">
                <ResponsiveContainer width={180} height={180}>
                  <PieChart>
                    <Pie data={occupancy?.donut || []} cx="50%" cy="50%"
                      innerRadius={55} outerRadius={80} dataKey="value" nameKey="name"
                    ...
                    <text x="50%" y="46%" textAnchor="middle" dominantBaseline="middle"
                      fill="var(--theme-text)" fontSize={18} fontWeight={700}>{occupancy?.occupancyRate ?? 0}%</text>
                    <text x="50%" y="58%" textAnchor="middle" dominantBaseline="middle"
                      fill="#94a3b8" fontSize={10}>Occupancy</text>
                  </PieChart>
                </ResponsiveContainer>
                <div className="flex flex-col gap-2 flex-1">
                  {(occupancy?.donut || []).map((d: any, i: number) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-bold text-theme-text">{d.percentage}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <ChartCard title="Bookings Overview" action={<PeriodSelect value={bookPeriod} onChange={setBookPeriod} />}>
              <ResponsiveContainer width="100%" height={210}>
          ...
          <div className="flex gap-4 flex-wrap">
            {/* Restaurant Revenue */}
            <ChartCard title="Restaurant Revenue" action={<PeriodSelect value={restPeriod} onChange={setRestPeriod} />}>
              <ResponsiveContainer width="100%" height={210}>
            ...
            <ChartCard title="Staff Performance (Tasks Completed)"
              action={<PeriodSelect value={staffPeriod} onChange={setStaffPeriod} options={["This Week", "This Month"]} />}>
              <div className="flex flex-col gap-3">
                {staffPerf.length === 0
                  ? <p className="text-theme-muted-light text-sm text-center py-8">No task data for this period</p>
                  : staffPerf.map((s: any, i: number) => (
                      <div key={i} className="flex items-center gap-3">
                        <span className="text-xs text-theme-muted w-24 shrink-0 text-right truncate">{s.name}</span>
                        <div className="flex-1 bg-theme-main rounded-full h-5 overflow-hidden">
                          <div className="h-full rounded-full flex items-center justify-end pr-2 transition-all duration-700"
                            style={{ width: `${Math.max((s.tasksCompleted / maxTasks) * 100, 8)}%`, background: STAFF_COLORS[i % STAFF_COLORS.length] }}>
                            <span className="text-[10px] font-bold text-theme-text">{s.tasksCompleted}</span>
                          </div>
                        </div>
                        <span className="text-xs font-bold text-theme-text w-6 shrink-0">{s.tasksCompleted}</span>
                      </div>
                    ))}
                {staffPerf.length > 0 && (
            ...
            <ChartCard title="Revenue by Department"
              action={<select className="bg-theme-main border border-theme-border rounded-lg px-2.5 py-1 text-xs text-theme-muted-light outline-none"><option>This Week</option></select>}>
              <div className="flex items-center gap-4">
                <div className="shrink-0">
                  <ResponsiveContainer width={160} height={160}>
                    <PieChart>
                      <Pie data={filteredRevByDept?.data || []} cx="50%" cy="50%"
                        innerRadius={50} outerRadius={75} dataKey="value" nameKey="name"
                        startAngle={90} endAngle={-270}>
                        {(filteredRevByDept?.data || []).map((_: any, i: number) => (
          ...
          <div className="flex gap-4 flex-wrap">
            <DataTable
              ...
              headers={["Date", "Rooms Revenue", "Restaurant Rev.", "Other Revenue", "Total"]}
              rows={revRows}
            ...
            <DataTable
              ...
              headers={["Date", "Occupied", "Available", "Reserved", "Occupancy %"]}
              rows={occRows}
              totalRow={occTotalRow}
              onViewFull={() => setFullModal({
                title: "Occupancy Report — Full Data",
                headers: ["Date", "Occupied", "Available", "Reserved", "Occupancy %"],
                rows: (occupancy?.table || []).map((r: any) => [r.date, r.occupied, r.available, r.reserved, `${r.occupancyPct}%`]),
                totalRow: occTotalRow ? ["Total", String(occTotalRow[1]), String(occTotalRow[2]), String(occTotalRow[3]), `${occupancy?.occupancyRate}%`] : undefined,
              })}
            />
            <DataTable
              title="Staff Report"
              headers={["Staff Name", "Department", "Tasks", "Efficiency %", "Rating"]}
              rows={staffRows}
              totalRow={staffAvg}
              onViewFull={() => setFullModal({
                title: "Staff Report — Full Data",
                headers: ["Staff Name", "Department", "Tasks Completed", "Efficiency %", "Rating"],
                rows: staffPerf.map(s => [s.name, s.department, s.tasksCompleted, `${s.efficiency}%`, s.rating]),
                totalRow: staffAvg ? ["Average", "—", String(staffAvg[2]), String(staffAvg[3]), String(staffAvg[4])] : undefined,
              })}
            />
          </div>
          ...
          <div className="bg-theme-card shadow-soft border border-theme-border rounded-2xl p-5 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-theme-text">Guest Satisfaction Trend</h3>
              <select className="bg-theme-main border border-theme-border rounded-lg px-2.5 py-1 text-xs text-theme-muted-light outline-none">
                <option>Weekly</option>
              </select>
            </div>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={satisfaction} margin={{ top: 24, right: 30, left: 10, bottom: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(128,128,128,0.2)" />
                <XAxis dataKey="name" tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} />
