...
export default function NewBookingWizard({ onClose }: Props) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (guestSearch.length > 2) {
        setIsSearching(true);
        api.get<any>(`/api/guests?search=${guestSearch}&limit=5&guestType=FOREIGN`)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const payload = {
        bookingType: 'FOREIGN',
        guest: { ...guestDetails, guestType: 'FOREIGN' },
        additionalGuests: additionalGuests.length > 0 ? additionalGuests : null,
        roomId: selectedRoomId,
        checkIn: `${stayDetails.checkIn}T${stayDetails.arrivalTime}:00.000Z`,
        checkOut: `${stayDetails.checkOut}T${stayDetails.checkOutTime}:00.000Z`,
        arrivalTime: `${stayDetails.checkIn}T${stayDetails.arrivalTime}:00.000Z`,
        ...
        {steps.map((s, idx) => {
          ...
          return (
            <div key={s} className="flex flex-col items-center gap-2 bg-theme-main px-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors ${isActive ? 'border-blue-500 bg-blue-500/10 text-blue-500' : isPast ? 'border-blue-500 bg-blue-500 text-white' : 'border-theme-border bg-theme-secondary text-theme-muted-light'}`}>
              ...
              <>
                <div className="w-full max-w-sm flex-shrink-0 snap-center bg-theme-secondary border border-theme-border rounded-2xl p-6 flex flex-col">
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div className="relative">
                      <label className="text-xs font-semibold text-blue-400 mb-1.5 block">Search Existing Guest</label>
                      <input 
                        type="text" 
                        placeholder="Type name, phone or email..."
                        value={guestSearch} 
                        onChange={e => setGuestSearch(e.target.value)} 
                        className="w-full bg-theme-card shadow-soft border border-primary/50 rounded-xl px-4 py-2.5 text-sm text-theme-text focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:border-blue-400" 
                      />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-xs font-semibold text-theme-muted mb-1.5 block">Country</label>
                        <input type="text" value={guestDetails.country} onChange={e => setGuestDetails({...guestDetails, country: e.target.value, id: ''})} className="w-full bg-theme-main border border-theme-border rounded-xl px-4 py-2.5 text-sm text-theme-text focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:border-primary" />
                      </div>
                    </div>
                        ...
                        {additionalGuests.map((ag, idx) => (
                          <div key={idx} className="flex flex-col gap-2 bg-theme-main p-3 rounded-xl border border-theme-border relative">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              />
                              <input 
                                type="text" placeholder="Passport Number" value={ag.idNumber} 
                                onChange={(e) => {
                                  const newArr = [...additionalGuests];
                ...
                <div className={`w-full max-w-sm flex-shrink-0 snap-center bg-theme-secondary border border-theme-border rounded-2xl p-6 flex flex-col ${step === 1 ? 'opacity-50 pointer-events-none' : ''}`}>
                  ...
                  <div className="space-y-4 flex-1">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-semibold text-theme-muted mb-1.5 block">Arrival Date *</label>
                      ...
                      <div>
                        <label className="text-xs font-semibold text-theme-muted mb-1.5 block">Stay Duration (Days)</label>
                        <input type="number" min="1" value={stayDetails.days} onChange={e => handleDaysChange(parseInt(e.target.value) || 1)} className="w-full bg-theme-main border border-theme-border rounded-xl px-4 py-2.5 text-sm text-theme-text focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:border-primary [color-scheme:dark]" />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-semibold text-theme-muted mb-1.5 block">Check-in Time *</label>
                      ...
                      <div>
                        <label className="text-xs font-semibold text-theme-muted mb-1.5 block">Check-out Time *</label>
                        <input type="time" value={stayDetails.checkOutTime} onChange={e => setStayDetails({...stayDetails, checkOutTime: e.target.value})} className="w-full bg-theme-main border border-theme-border rounded-xl px-4 py-2.5 text-sm text-theme-text focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:border-primary [color-scheme:dark]" />
                      </div>
                    </div>
                    <div>
                  ...
                  {availableRooms.length === 0 ? (
                    ...
                    availableRooms.map(room => (
                      <div 
                        ...
                        className={`flex gap-4 p-3 rounded-xl border cursor-pointer transition-colors ${selectedRoomId === room.id ? 'border-blue-500 bg-blue-500/10' : 'border-theme-border bg-theme-main hover:border-theme-strong'}`}
                      >
                        ...
                        <div className="flex items-center">
                          <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${selectedRoomId === room.id ? 'border-blue-500 bg-blue-500 text-white' : 'border-theme-border'}`}>
                          ...
                          {['Cash', 'Credit / Debit Card', 'Bank Transfer', 'Digital Wallet'].map(method => (
                            <label key={method} className="flex items-center gap-3 cursor-pointer">
                              <input 
                                ...
                                className="w-4 h-4 text-primary bg-theme-main border-theme-border focus:ring-blue-500"
                              />
